...
%        require normlized data, in case input data is not already  normalized
% 
% 1: RMS
% 2: Coastline
% 3: Band power
...
if nargin < 3
    ...
    if isfield(params, 'm')
        if length(params.m) ~= size(data,2) && params.outputNames == false
...
%         freq = out.freq';
    ...
    if params.features(21)
        data_spikes = data;
        if ~isempty(m)
            data_spikes = data_spikes - repmat(m,[size(data_spikes,1),1]);
        end
        if ~isempty(sd)
            data_spikes = data_spikes./repmat(sd,[size(data_spikes,1),1]);
        end
        nspikes = zeros(nChans,1);
        for i_chan = 1:nChans
            spikes = findSpikes(data_spikes(:,i_chan), Fs, [6 100 12]);
    ...
    if params.features(24)
        ...
        if ~isempty(m)
            data_ft.trial{1} = data_ft.trial{1} - repmat(m',[1,size(data_ft.trial{1},2)]);
        end
        if ~isempty(sd)
            data_ft.trial{1} = data_ft.trial{1}./repmat(sd',[1,size(data_ft.trial{1},2)]);
        end
        data_ft.fsample = Fs;
        label = cell(1,nChans);
        data_ft.label = label;
        ...
        data_ft.sampleinfo = [1 size(data,1)];
        for i_chan = 1:nChans
            HFO_temp = HFO_detection(data_ft, 2, i_chan, 'Ripples');
